...
describe("assertAdmin", () => {
  it("throws when no admin row", async () => {
    ...
    await expect(assertAdmin(ADMIN_UUID, admin)).rejects.toThrow(/Forbidden/);
  });

  it("returns the admin client when row exists", async () => {
    const admin = mockSupabaseAdmin();
    queueAdminCheck(admin, true);
    await expect(assertAdmin(ADMIN_UUID, admin)).resolves.toBe(admin);
  });

  it("surfaces query errors", async () => {
    const admin = mockSupabaseAdmin();
    admin.queue("admins", { data: null, error: { message: "boom" } });
    await expect(assertAdmin(ADMIN_UUID, admin)).rejects.toThrow("boom");
  });
});
...
describe("adminListTournamentsHandler", () => {
  it("rejects non-admins", async () => {
    const admin = mockSupabaseAdmin();
    queueAdminCheck(admin, false);
    await expect(adminListTournamentsHandler(admin, ADMIN_UUID)).rejects.toThrow(/Forbidden/);
  });
  ...
  it("returns rows on success", async () => {
    ...
    const rows = await adminListTournamentsHandler(admin, ADMIN_UUID);
  ...
  it("surfaces query error", async () => {
    ...
    await expect(adminListTournamentsHandler(admin, ADMIN_UUID)).rejects.toThrow("db down");
  });
});
...
describe("adminGetTournamentHandler", () => {
  it("returns the row", async () => {
    ...
    const row = await adminGetTournamentHandler(admin, ADMIN_UUID, { id: T_UUID });
...
describe("adminListTeamsHandler", () => {
  it("returns rows", async () => {
    ...
    const rows = await adminListTeamsHandler(admin, ADMIN_UUID, { tournamentId: T_UUID });
...
describe("listMyCaptainTeamsHandler", () => {
  it("returns [] when no email claim", async () => {
    const admin = mockSupabaseAdmin();
    const rows = await listMyCaptainTeamsHandler(admin, undefined);
    expect(rows).toEqual([]);
  ...
  it("queries with lowercased email", async () => {
    const admin = mockSupabaseAdmin();
    admin.queue("teams", { data: [{ id: "t1" }], error: null });
    const rows = await listMyCaptainTeamsHandler(admin, "Foo@BAR.com");
    expect(rows).toEqual([{ id: "t1" }]);
  ...
  it("bubbles errors", async () => {
    ...
    await expect(listMyCaptainTeamsHandler(admin, "a@b.co")).rejects.toThrow("nope");
  });
});
...
describe("adminGetScoreAuditHandler", () => {
  it("returns shaped result", async () => {
    ...
    const res = await adminGetScoreAuditHandler(admin, ADMIN_UUID, { tournamentId: T_UUID });
    expect(res).toEqual({
      entries: [{ id: "a1" }],
      teams: [{ id: "t1", name: "X" }],
  ...
  it("throws on any sub-query error", async () => {
    ...
    await expect(
      adminGetScoreAuditHandler(admin, ADMIN_UUID, { tournamentId: T_UUID }),
...
describe("adminCloneTournamentHandler", () => {
  ...
  it("throws when source missing", async () => {
    ...
    await expect(adminCloneTournamentHandler(admin, ADMIN_UUID, input)).rejects.toThrow(
  ...
  it("clones settings and copies holes", async () => {
    ...
    const res = await adminCloneTournamentHandler(admin, ADMIN_UUID, input);
  ...
  it("skips hole insert when source has no holes", async () => {
    ...
    await adminCloneTournamentHandler(admin, ADMIN_UUID, input);
